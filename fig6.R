...
results <- read.table(paste0('results/pathway_analysis/gsea_results/gsea_results_AMP_RATIOS/gsea_report_for_na_neg_1747673097610.tsv'), header = TRUE, sep ='\t', row.names=1)
...
results <- read.table(paste0('results/pathway_analysis/enrichr_results/enrichr_results_PROTECTED.txt'), header = TRUE, sep ='\t', row.names=1)
...
# pathway names need to match formatting of GSEA 
...
scatter_plot <- ggplot(plot_data, aes(x = x, y = y, color = neg_log_enrichr_q_values)) +
  ...
  scale_color_gradient(low = "black", high = "#0000FF", name = "-log10(Enrichr q-values)")

filename <- paste0("plots/fig6/gsea_amp_ratio_vs_enrichr_higher_amp_DR_", data_label, ".png")
...
scatter_plot <- ggplot(plot_data, aes(x = x, y = y, color = neg_log_enrichr_q_values)) +
  ...
  scale_size_continuous(name = NULL, range = c(3, 10)) +  # legend title removed
  scale_color_gradient(low = "black", high = "#0000FF", name = NULL) +  # legend title removed
  scale_x_continuous(expand = expansion(mult = 0.2)) +
  scale_y_continuous(expand = expansion(mult = 0.2)) +
  theme_minimal(base_size = 14) +
  ...
  theme(
    ...
    legend.title = element_blank()  # just in case any titles remain
  )


filename <- paste0("plots/fig6_NOLABELS/gsea_amp_ratio_vs_enrichr_higher_amp_DR_", data_label, ".png")
...
## FIG 5B,C - E2F TARGETS, RELATIVE AMPLITUDE
...
p <- ggplot(df_combined, aes(x = value, fill = group)) +
  ...
  labs(title = paste0("Relative Amplitude Distributions - E2F Targets (",length(results_EXPOSED$rel_amp)," Transcripts)" ),
...
filename <- "plots/fig6/amp_dists_E2F_TARGETS.png"
...
filename <- "plots/fig6_NOLABELS/amp_dists_E2F_TARGETS.png"
...
p <- ggplot(df, aes(x = log2(value))) +
  ...
  labs(title = paste0( "Relative Amplitude Ratio - E2F Targets (",length(results_EXPOSED$rel_amp)," Transcripts)"),
...
filename <- "plots/fig6/amp_ratios_E2F_TARGETS.png"
ggsave(filename, plot = p, width = 8, height = 6, units = "in", dpi = 300)
...
filename <- "plots/fig6_NOLABELS/amp_ratios_E2F_TARGETS.png"
ggsave(filename, plot = p, width = 4, height = 4, units = "in", dpi = 300)
########################################################################################
